   ...
   for (int y=0; y<grid->get_num_staves(); y++)
   {
      ...
      for (int x=0; x<grid->get_num_barlines(); x++)
      {
         ...
         for (auto &floating_measure : FloatingMeasure::find_at_staff_and_barline(staff->get_id(), x))
         {
            ...
            bool is_focused = (focused_floating_measure_id == floating_measure->get_id() && is_measure_target);
            Measure::Base *floating_measure_measure = Measure::find(floating_measure->get_measure_id(), Measure::FIND_OPTION_RAISE_NOT_FOUND);
            MeasureRenderComponent measure_render_component(context_measure, floating_measure_measure, music_engraver, full_measure_width, x_pos + floating_measure_x_offset, y_counter, row_middle_y, this_staff_height, showing_debug_data, is_focused, note_cursor_x);
