...
void GUIScoreEditor::on_draw()
{
   ...
   BasicMeasure *measure = get_measure_at_cursor();
...
BasicMeasure *GUIScoreEditor::get_measure_at_cursor()
{
...
{
   BasicMeasure *focused_measure = get_measure_at_cursor();
...
float GUIScoreEditor::get_measure_length_to_note(BasicMeasure &measure, int note_index)
{
   float sum = 0;
...
float GUIScoreEditor::get_measure_width(BasicMeasure &m)  // TODO: should probably use a helper
{
   float sum = 0;
...
{
   BasicMeasure *current_measure = measure_grid.get_measure(measure_cursor_x, measure_cursor_y);
   if (!current_measure)
   {
      note_cursor_x = 0;
   }
