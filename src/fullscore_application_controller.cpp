   ...
   , yank_measure_buffer()
{
   ...
   BasicMeasure *m = current_gui_score_editor->measure_grid.get_measure(0, 0);
   m->set_notes({Note(2), Note(0), Note(1)});

   BasicMeasure *dm = current_gui_score_editor->measure_grid.get_measure(0, 1);
...
Action::Base *FullscoreApplicationController::create_action(std::string action_name)
{
   ...
   BasicMeasure *focused_measure = nullptr;
